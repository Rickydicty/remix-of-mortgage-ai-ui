...
const ClientRatesTab = () => {
  ...
  const validateAndSearch = () => {
    ...
    if (!houseValue) {
      validationErrors.push({ field: 'houseValue', message: 'House value is required' });
    }
    if (!mortgageAmount) {
      validationErrors.push({ field: 'mortgageAmount', message: 'Mortgage amount is required' });
    }
    if (!term) {
      validationErrors.push({ field: 'term', message: 'Mortgage term is required' });
    }
    if (!berRating) {
      validationErrors.push({ field: 'berRating', message: 'BER rating is required' });
    }

    // Value validation
    if (house > 0 && house < 50000) {
      validationErrors.push({ field: 'houseValue', message: 'House value seems too low' });
    }
    if (mortgage > 0 && mortgage < 20000) {
      validationErrors.push({ field: 'mortgageAmount', message: 'Mortgage amount seems too low (minimum €20,000)' });
    }

    // LTV validation (90% max for first time buyers)
    if (house > 0 && mortgage > 0) {
      ...
      if (mortgage > house) {
        ...
        validationErrors.push({ 
          field: 'mortgageAmount', 
          message: `LTV - Maximum 90% loan-to-value allowed for first time buyers. Your LTV is ${ltv.toFixed(1)}%` 
        });
      }
    }
    ...
    // Term validation
    if (years > 0 && years < 5) {
      validationErrors.push({ field: 'term', message: 'Minimum mortgage term is 5 years' });
    }
    if (years > 35) {
      validationErrors.push({ field: 'term', message: 'Maximum mortgage term is 35 years' });
    }

    setErrors(validationErrors);

    if (validationErrors.length === 0) {
      const searchResults = searchMortgages(mortgage, house, years, berRating);
  ...
  const searchMortgages = (mortgage: number, house: number, years: number, berRatingValue: string): SearchResult[] => {
    ...
    const eligibleRates = mortgageRates.filter(rate => {
      if (ltv <= rate.ltv) {
        ...
        // If showing cashback only, filter
    ...
    // Calculate monthly payments, total costs, and cashback amounts
